...
| L179 | S2 | After the L177 redesign, time-span windows spanned 10x in the coarse part of mixed data, and Hampel with differencing and increments z-score never tested coarse records | `meteoscreening.py` | [x], rolling and difference tests run per resolution period |
| L180 | S2 | Records off the finest resolution's grid (15 min after 10 min) were dropped without a message | `meteoscreening.py` | [x], grid step is the common divisor of resolutions and offsets; timestamp jitter is guarded (L186) |
| L181 | S4 | An overlapping resolution change counted more than 100 % coverage in resampling | `meteoscreening.py` `resample` | [x] |
| L182 | S4 | Re-running finalize with looser thresholds did not restore records removed by a stricter run | `meteoscreening.py` | [x] |
| L183 | S3 | `resample('1D')` raised `TypeError: Day < Minute` (pandas 3) | `meteoscreening.py` `resample` | [x] |
| L184 | S5 | Manual removal and `correction_setto_value` dates that match no record were silent | `meteoscreening.py` | [x], warns |
| L185 | S4 | QCF reports counted the empty slots between coarse records as missing records (45 % "missing" for one missing record) | `qaqc/qcf.py`, `meteoscreening.py` | [x], rows with NaN in the missing-values flag (and, since L190, a NaN value) are not records; a missing coarse record counts once |
| L186 | S2 | The common-divisor grid (L180) had no guard: three records 1 s late shrank it to a 1 s grid (millions of rows, MemoryError at 1 ms) | `meteoscreening.py` `_grid_seconds` | [x], raises when off-phase records shrink the grid below 1 s or above 20 slots per record |
| L187 | S2 | A correction after finalize rebased the detector onto the masked series, so later tests skipped removed records and a looser re-finalize restored them untested | `meteoscreening.py` `_set_corrected` | [x] |
| L188 | S5 | Per-period warnings (short period, untested records) never printed with default arguments | `meteoscreening.py` | [x] |
| L189 | S4 | Resolution periods of 2 records got flag 0 though nothing was tested; periods of 1 record NaN | `meteoscreening.py` | [x], fewer than 3 records stay untested with a warning |
| L190 | S4 | `FlagQCF` treated a row with a measured value but NaN missing-values flag as "not a record", so a hard-flagged spike passed | `qaqc/qcf.py` | [x], a row is "not a record" only if its value is NaN too |
| L191 | S5 | Tiny irregular input where frequency detection keeps no record crashed with an unrelated error (HEAD: `ValueError: min() iterable argument is empty`) | `meteoscreening.py` | [x], clear `ValueError` |
| L192 | S1 | GUI Meteo screening tab kept TIMESTAMP_END while potrad, the day/night split and the zero-offset correction expect MIDDLE: on 30-min data 6 of 144 records got the wrong day/night class, SW_IN_POT off by up to 54 W m-2 (single resolution too) | `gui/tabs/meteo_screening.py` | [x], the tab runs through `StepwiseMeteoScreeningDb` and uses the project's UTC offset (the only offset in the GUI) |
| L193 | S2 | GUI Meteo screening tab, mixed resolutions: records dropped by the majority-resolution grid, the coarse period lost at resampling, unweighted means, coarse spikes skipped by Hampel with differencing, empty slots counted in the QCF and filled by the zero-offset correction | `gui/tabs/meteo_screening.py`, `_screening_base.py` | [x], as L192 |
| L194 | S3 | GUI Meteo screening tab: a download without a majority resolution raised inside a slot; the tab kept the previous field's data and status | `gui/tabs/meteo_screening.py:188` | [x], as L192 |
| L195 | S5 | GUI screening tabs: absolute limits missing from the step picker | `gui/widgets/stepwise_method_params.py` | [x] |
| L196 | S2 | GUI screening tabs: saved correction rows were lost on project restore; `CorrectionsPanel.set_state` stored the rows, then `set_measurement` rebuilt the panel and overwrote them with defaults | `gui/widgets/corrections_panel.py` | [x] |
| L197 | S4 | GUI Meteo screening tab: Copy Python wrote a plain `StepwiseOutlierDetection` script instead of the `StepwiseMeteoScreeningDb` workflow | `gui/tabs/meteo_screening.py` | [x], library codegen `meteoscreening_to_code` |
| L198 | S4 | GUI Meteo screening tab: site coordinates were fixed when the download was loaded; later changes in Project settings had no effect | `gui/tabs/meteo_screening.py` | [x] |
