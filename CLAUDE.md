...
| SWINGapFillerXGBoost | Yes | SW_IN_POT + timestamps (+ opt. `context_df` drivers) | SW_IN w/ nighttime constraint; `nighttime_threshold=0.001` matches `remove_nighttime_zero_offset`. **Climatology ceiling** (below); ships its own XGBoost + FeatureEngineer defaults (`_XGB_DEFAULTS`/`_FE_DEFAULTS`); lags off by default, SW_IN_POT excluded from rolling/EMA |
