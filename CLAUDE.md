...
| `dv.flux` | `FluxConfig`, `FluxLevelData`, `run_chain`, `init_flux_data`, `add_driver`, `WindDoubleRotation`, `reynolds_decomposition`, `MaxCovariance`, `PreWhiteningBootstrap`, `PwbBatchDetection`, `TlagApplier`, `PerFilePipeline`, `process_one_file`, `FluxDetectionLimit`, ustar classes, **four NEE→GPP+RECO partitioning variants** (faithful ports distinguished by a token after the `_NT` nighttime / `_DT` daytime suffix — `_OF` ONEFlux / `_RP` REddyProc — so all coexist in one dataframe): `NighttimePartitioningOneFlux` / `partition_nee_nighttime_oneflux` (ONEFlux Reichstein 2005 port, per-calendar-year, emits `*_NT_OF` incl. robust `*_NT_OF_ROB`; helpers `lloyd_taylor`, `sunrise_sunset`), `NighttimePartitioningReddyProc` / `partition_nee_nighttime_reddyproc` (REddyProc `sMRFluxPartition` port, **whole-record single E0**, signature adds `lon`/`utc_offset`, potential-radiation day/night split, Kelvin Lloyd-Taylor, no robust variant, emits `*_NT_RP`; helpers `lloyd_taylor_kelvin`, `potential_radiation`; reproduces the ReddyProc reference columns 1:1, RECO r≈0.997 on CH-DAV), and `DaytimePartitioningReddyProc` / `partition_nee_daytime_reddyproc` (REddyProc `partitionNEEGL` / Lasslop 2010 light-response-curve port: per-window rectangular-hyperbola LRC fit with E0 fixed from a GP-smoothed nighttime estimate, then distance-weighted RECO/GPP interpolation; signature `nee` measured + `ta`/`vpd`/`sw_in` gap-filled drivers + `lat`/`lon`/`utc_offset`, optional `nee_sd`, `vpd_in_kpa=True`; emits `*_DT_RP` incl. fitted `K`/`BETA`/`ALPHA`/`RREF`/`E0_DT_RP`. Re-implements R's `nls`, `optim` BFGS and the `mlegp` GP to reach REddyProc fidelity — not bit-exact end-to-end (a stack of 3 nested optimizers), but matches a fresh REddyProc run on identical inputs to RECO r≈0.999, GPP r≈0.9999; the larger gap to the bundled `Reco_DT_CUT_REF`/`GPP_DT_CUT_REF` is reference provenance, not algorithm; bootstrap `*_SD`/CUT_16/84 not yet emitted), and `DaytimePartitioningOneFlux` / `partition_nee_daytime_oneflux` (ONEFlux `flux_part_gl2010` / Lasslop 2010 port, FLUXNET2015 daytime: per 4-day/2-day window LRC fit via penalized `scipy.leastsq` over a 5-model cascade — HLRC_LloydVPD→_Lloyd→_Lloyd_afix/_LloydVPD_afix→LloydT_E0fix — with E0 from a wider nighttime window, an internal Reichstein-MDS NEE-uncertainty look-up as the fit weight, then distance-weighted interpolation + Jacobian GPP SE. **Day/night split is the measured-`Rg`≤4/>4 threshold — NO latitude/solar geometry** (unlike the nighttime port). Signature is the faithful measured+filled set `nee`/`ta`/`sw_in` measured + `ta_f`/`sw_in_f`/`vpd` filled, `vpd_in_kpa=True`; emits `*_DT_OF` incl. `SE_GPP_DT_OF` + central-record `ALPHA`/`BETA`/`K`/`RREF`/`E0_DT_OF`. **Stores working arrays as float32 to match ONEFlux** (FLOAT_PREC); validated vs a native-ONEFlux run on identical CH-DAV arrays to RECO r≈0.999, GPP r≈0.9999 — the gap to the REddyProc-derived bundled `*_DT_CUT_REF` is provenance; bootstrap percentile uncertainty not yet emitted; ~22s/year, bottleneck is the per-window `scipy.leastsq` cascade — `_build_predict` precomputes the fixed Lloyd-Taylor temp factor once per fit, a bit-identical speedup), all in `diive.flux.partitioning`. The same four partitioning ports are wired into the flux chain as **Level 4.2** (composable callables `run_level42_nighttime_oneflux` / `run_level42_nighttime_reddyproc` / `run_level42_daytime_reddyproc` / `run_level42_daytime_oneflux`, mirroring the four L4.1 gap-fillers): each runs one partitioning per USTAR scenario and merges its `*_NT_OF`/`*_NT_RP`/`*_DT_RP`/`*_DT_OF` columns into `fpc_df` with the scenario label appended (`RECO_NT_OF_CUT_50`); meteo driver columns resolve from `data.full_df`, `lat`/`lon`/`utc_offset` from `data.meta`; the **nighttime** variants read their gap-filled NEE from a selectable L4.1 method (`gapfill_method='mds'` default), the **daytime** variants use measured NEE only. Exposed on `run_chain` via `FluxConfig.partition_*` fields, with `partitioned_cols()` lookup and `levels.level42_*` instance access. Per-level `run_level*`, `make_level32_detector`, and `chain_to_code` / `level2_to_code` … `level41_to_code` / `level42_to_code` (render chain choices as a reproducible script) live in `diive.flux.fluxprocessingchain`. **Random measurement uncertainty:** `RandomUncertaintyPAS20` (`diive.flux.lowres.uncertainty`) — per-record random uncertainty (Pastorello 2020 / faithful ONEFlux `randunc` port), a **4-method hierarchical cascade** (first method to succeed wins) emitting `{flux}_RANDUNC` + `WINDOW_N_VALS_METHOD1..4`: **(1)** std of >5 meteo-similar measured fluxes in a ±7 d/±1 h window (MDS similarity via the shared `gapfilling.similarity` kernel — the only *direct* measurement); **(2, ONEFlux)** median of method-1 uncertainties of ±20%-similar fluxes (floor 2 µmol) in a ±14 d window; **(3) & (4)** diive extensions (median over the whole record / nearest-magnitude fluxes) so every record gets a value. `.run(progress_callback=)`; `.randunc_series` / `.randunc_results` / `.randunc_results_cumulatives`. The 4 per-record hot loops are **vectorised numpy** (searchsorted date windows, ~35x faster than the old per-record pandas loops, bit-identical). Cumulative propagation = **quadrature** √Σσ² of independent random errors, computed NaN-safe (`sqrt((randunc**2).cumsum())`, **not** an object-dtype `ufloat` cumsum — a single NaN must not poison the tail). Codegen `randunc_to_code` in `diive.flux.lowres.codegen`. **Joint uncertainty:** `JointUncertaintyPAS20` + `joint_uncertainty_pas20` (same module) — faithful ONEFlux `compute_join` port combining the random uncertainty with the scenario-ensemble (USTAR-filtering / energy-balance) percentile spread in quadrature per record: `JOINTUNC = √(RANDUNC² + ((upper−lower)/divisor)²)`. `divisor` = `JOINT_DIVISOR_1SIGMA=2.0` for NEE's 16th/84th USTAR scenarios (e.g. `NEE_CUT_16_f`/`NEE_CUT_84_f` → `NEE_CUT_REF_JOINTUNC`), `JOINT_DIVISOR_IQR=1.349` for LE/H's 25th/75th IQR. NaN in any input → NaN (ONEFlux INVALID_VALUE). Default output name strips a trailing `_RANDUNC`→`_JOINTUNC`. `.run()`; `.jointunc_series`/`.jointunc_results` (random + scenario terms) `/.jointunc_results_cumulatives`. Cumulative propagation treats the two sources by their nature: random error **independent → quadrature** `sqrt(cumsum(randunc²))`, scenario (threshold) choice **fully correlated → running spread** `(cumsum(upper)−cumsum(lower))/divisor`, then combined in quadrature. Codegen `jointunc_to_code`. |
| `dv.analysis` | `DailyCorrelation`, `GrangerCausality`, `StratifiedAnalysis`, `GapFinder`, `GapStats`, `GridAggregator`, `Histogram`, `FindOptimumRange`, `SeasonalTrendDecomposition`, `BinFitterCP`, `CompoundExtremes` (classify months/days into compound-extreme categories from two drivers' z-scores — after Wang et al. Fig. 2: high-VPD = *air* extreme, low-SWC = *soil* extreme, both = *compound*; each var has a configurable extreme direction `high`/`low` + threshold; `agg=` monthly/daily, `standardize_by=` `season` (deseasonalized per calendar month/day-of-year, default) or `record`; `.results`/`.counts`/`.labels`; pairs with `dv.plotting.CompoundExtremesPlot`; `diive/analysis/compoundextremes.py`), `harmonic_analysis`, `spectrogram`, `percentiles101`, `rank_drivers`, `profile_dataframe` (per-variable profiling table — one row per column: dtype/count/missing/gaps/unique/zeros/constant + numeric summaries), `dataframe_overview` (dataset-level facts: rows/cols/duplicate timestamps/missing %/freq/span/memory), `count_gaps` |
...
| `dv.plotting` | `HeatmapDateTime`, `HeatmapXYZ`, `HeatmapYearMonth`, `HexbinPlot`, `ScatterXY`, `TimeSeries`, `DielCycle`, `RidgeLinePlot`, `HistogramPlot`, `ShiftedDistributionPlot`, `Cumulative`, `CumulativeYear`, `LongtermAnomaliesYear`, `TreeRingPlot`, `CompoundExtremesPlot` (quadrant scatter of two standardized drivers' z-scores, coloured/marked by compound-extreme category with dashed threshold lines and per-point period annotations — the Wang et al. Fig. 2 view of `dv.analysis.CompoundExtremes`; two-phase, `from_compound_extremes(ce)` builds it straight from a fitted analysis, or pass x/y/category for pre-classified data; `category_styles=` overrides colour/marker/label per category code `none`/`var1`/`var2`/`compound`; `diive/core/plotting/compoundextremes.py`), `WindRosePlot` (radial polar-bar plot aggregating a variable into wind-direction sectors — `agg=` mean/median/min/max/sum/std/count, `n_sectors=` 4/8/16 compass labels else degree labels, with per-sector labels auto-replaced by a fixed ring of 16/8 compass bearings past `max_sector_labels=16` so many-sector roses stay readable; North-up clockwise meteorological layout, bars anchored at the zero line so negative aggregates like CO2 uptake hang inward of the zero circle; `show_colorbar=False` drops the colorbar (radial value scale remains), `sector_label_pad=` keeps compass labels clear of the plot ring, `cb_digits_after_comma=None` auto-picks integer-vs-float colorbar ticks; optional `z=`/`z_agg=` second variable colours the bars (length stays `series`, colour = per-sector z aggregate, added as a `Z` results column); per-sector `.results` DataFrame + Rich `report()`; `diive/core/plotting/windrose.py`), `FormatStyle` (shared plot **chrome** — title/labels/units/fontsizes/weights/text-spine-tick-grid colors/grid/legend/zeroline — and the **only** way to set chrome: pass `plot(format_style=FormatStyle(...))`. `None` fields resolve to the `LightTheme` standard block (`FONTSIZE_TITLE`/`FONTSIZE_AXLABEL`/`FONTSIZE_TICKS`/`COLOR_TEXT`/`COLOR_CHROME`/`COLOR_FACE`), so a bare `FormatStyle()` **is** the house style and editing those constants restyles every plot at once. The old flat chrome kwargs (`title`/`xlabel`/`ylabel`/`series_units`/`axlabels_fontsize`/...) were **removed** from every `plot()` (v0.91.0 breaking change) — use `format_style=`, and `.merged(**overrides)` to vary one field off a shared style. Data-render args (`color`/`cmap`/`marker`/`vmin`/`vmax`/...) and colorbar args (`cb_*`, and the colorbar-label `zlabel` on heatmaps/`HexbinPlot`) stay direct `plot()` kwargs. The matplotlib work lives in `FormatStyle.apply(ax, default_title=, default_xlabel=, default_ylabel=, zeroline_data=)`, reusing the `plotfuncs` helpers, so there's one implementation of "the look". Lives in `diive/core/plotting/styles/format.py`. Conversion is chrome-only for the multi-axes `RidgeLinePlot` and polar `TreeRingPlot` (gridspec/polar machinery untouched). **GUI:** the plot-settings panel has one shared **Format** section (title/labels/units/font sizes/grid/legend) on every plot type, read via `_format_values()` -> `values()["_format"]` -> `FormatStyle(**...)` in `tabs/plotting.py`) |
...
Top-level (no namespace): `load_exampledata_parquet`, `load_exampledata_parquet_lae`, `load_parquet`, `load_parquet_many` (read + `combine_first`-merge several parquet files; optional `progress_callback(phase, done, total, filepath)` — parquet counterpart to `MultiDataFileReader`), `save_parquet` (with `enforce_diive_format=True` → single header row + valid `TIMESTAMP_*` index name), `to_diive_format`, `ReadFileType`, `search_files`, `sstats`, `keep_vars` (non-destructive column subselection — column analogue of `times.keep_daterange`), `keep_records_where` (non-destructive conditional *row* subselection: select records of a target var where a *condition* var falls within `[lower, upper]`; either bound optional, `inclusive=` boundary control, `invert=True` keeps records *outside* the range, `set_to_nan=True` default masks out-of-range to NaN to preserve the index else drops them; companion `select_records_to_code` renders a chain of keep/remove selections as a script — both in `diive/core/dfun/frames.py`), `transform_yearmonth_matrix_to_longform`, `get_encoded_value_from_int`, `get_encoded_value_series`
...
**MDS faithful ONEFlux port** (`FluxMDS`, `diive/gapfilling/mds.py`): the gap-fill cascade lives once in
`diive/gapfilling/similarity.py::mds_gapfill_cascade` — the ONEFlux 6-stage expanding-window MDS (ported from
`uncert_via_gapFill`, validated to fill r≈0.9997 vs a native ONEFlux run on CH-DAV), plus `meteo_similar_mask`,
`mds_quality_from` (1/2/3), `mds_granular_flag`. The **same cascade** backs the daytime-partitioning NEE uncertainty
(`daytime_oneflux._uncert_via_gapfill`), so it is **dtype-preserving** for the tolerance comparisons: a float32 caller
gets ONEFlux's f4 boundary behaviour, a float64 caller (MDS) full precision; the mean/SD reduction follows the input
dtype. `avg_min_n_vals` default **2** (ONEFlux `>1`; the uncertainty path uses 10); SD is N-1; sym_mean is the optional
Vekuri (2023) variant. The public flag `FLAG_*_gfMDS_ISFILLED` is **granular** `method*1000 + time_window` (0=measured;
1/2/3=ALL/SWIN/MDC method), with the faithful 1/2/3 quality kept in `.PREDICTIONS_QUALITY`. `FluxMDS` and
`RandomUncertaintyPAS20` both take **`vpd_in_kpa=True`** (diive kPa convention; converted to hPa internally for the
faithful 5-hPa / 0.5-kPa tolerance — pass `False` for an hPa column). (There is no longer a separate `_FluxMDS` class.)
`RandomUncertaintyPAS20` (`diive/flux/lowres/uncertainty.py`) methods 1 & 2 are faithful `randunc.c` ports (1 = ±7d/±1h
meteo-similar SD, >5 samples; 2 = ±14d median of method-1 uncertainties within ±20%/floor-2); methods 3 & 4 are
documented diive extensions (ONEFlux leaves those records undefined). For these ports, required units are **stated in
the docstrings, not validated** — don't add median-guessing warnings or EddyPro-specific "hPa, divide by 10" hints to
the library (EddyPro's convention can change; the caller is responsible for units).
...
6-level eddy covariance post-processing: L2 (quality flags) → L3.1 (storage correction) → L3.2 (outlier removal) → L3.3 (USTAR filtering) → L4.1 (gap-filling) → L4.2 (NEE→GPP+RECO partitioning, optional).
...
- **Menu tabs are multi-instance.** Tabs opened from menus (`registry.MENU_TABS`, factories) open a NEW numbered
  ...
  `registry.SINGLE_INSTANCE_TABS` instead focus the existing one — that set is reserved for the **three app-wide
  singleton editors** (`Appearance`, `Project settings`, `Metadata explorer`), each of which edits one shared singleton
  (`theme.manager` / `site.manager` / the metadata `manager`) so a second copy would show conflicting state; keeping
  `Metadata explorer` single also lets the `_edit_metadata` "Edit metadata…" relay stay a trivial focus-and-select.
  **Every other menu tab opens a second instance** (numbering inherited automatically — no per-tab code). Always-on tabs have their close
...
- **Select records by condition tab** (`tabs/select_records.py`, `Data ▸ Select records by condition`, multi-instance) —
  filter a **target** variable by the value of a **condition** variable, the row-on-a-condition sibling of *Select
  variables* / *Select date range*. Iterative: builds an **operation** (condition combo + lower/upper with open-bound
  toggles + `inclusive` + an **action** Keep-selected / Remove-selected) and **Select records** applies it to a working
  keep-mask; operations **stack** (chain several conditions/ranges/actions), with **Undo last** / **Reset**. The target
  may also be its own condition. All range logic is the library's `dv.keep_records_where` (the *remove* action passes
  `invert=True`); the working series = `target.where(keep_mask)`, computed synchronously (cheap vectorized boolean ops —
  no `WorkerRunner`). Two-panel preview (condition + editable `[lower,upper]` band + last-op markers on top, faint full
  target + surviving **kept** records on the bottom), a hero strip (KEPT / OF VALID / SHARE / STEPS) and an applied-steps
  list. **Add selection to dataset** emits `{target}_SEL` (DERIVED provenance recording every step); **Copy Python** →
  library `select_records_to_code` (one `keep_records_where` line per step, chaining via `df.assign`, `invert=True` for
  remove). `save_state` persists only the target + current operation editor (the accumulated steps rebuild
  interactively). All algorithms are library work.
...
- **Random uncertainty (PAS20) tab** (`tabs/uncertainty_randunc.py`, `Flux ▸ Random uncertainty (PAS20)`,
  single-instance, separated from the partitioning tabs by a menu separator). Standalone `DiiveTab` sharing the chrome
  (`build_titlebar` + `CopyPythonButton`, `WorkerRunner`, `_auto_pick`) — **not** a `BasePartitioningTab` subclass
  (single output column, not GPP/RECO). Layout mirrors the **correction tabs**: a fixed-width Settings column (header ->
  description -> 5 input combos with ✓/✗ markers + VPD-in-kPa toggle -> Run -> status/progress -> Add) beside a method
  **hero** band (mean / median / records / cumulative ±σ, reusing the Overview `_chip`/`_MetricSlot`/`_stat_separator`)
  over a **3-panel** preview: flux ±σ band (full top row), cumulative ±σ bounds (bottom-left), and a measured-flux vs
  random-uncertainty scatter (bottom-right) — the scatter is **method-1 records only** (`WINDOW_N_VALS_METHOD1 >= 6`) so
  the method 2-4 repeated-median fallbacks don't paint as horizontal streaks (the genuine Hollinger & Richardson 2005
  flux-magnitude scaling). Progress bar driven by the library `RandomUncertaintyPAS20.run(progress_callback=)` via a
  queued worker->GUI signal. **Add result** emits the single `{flux}_RANDUNC` column (DERIVED, parent = measured flux);
  **Copy Python** -> `randunc_to_code`. All computation is library work.
- **Joint uncertainty (PAS20) tab** (`tabs/uncertainty_jointunc.py`, `Flux ▸ Joint uncertainty (PAS20)`, after the
  Random uncertainty tab). Standalone `DiiveTab` sharing the same chrome as the random tab. Combines a per-record
  `{flux}_RANDUNC` (run the Random uncertainty tab first) with the scenario-ensemble percentile spread via the library
  `JointUncertaintyPAS20`. A **Scenario percentiles** combo picks the convention -> divisor + scenario auto-pick needles:
  *NEE — USTAR scenarios (16th/84th ÷2)* or *Energy flux LE/H (25th/75th ÷IQR 1.349)*; scenario auto-pick is biased
  toward the random-uncertainty column's flux base (`_scenario_prefer()`, so a NEE randunc seeds `NEE_CUT_16/84`, not
  GPP). 4 input combos (randunc, lower/upper scenario, gap-filled flux) with ✓/✗ markers. **3-panel** preview: flux ±
  joint band (top), random-vs-scenario component decomposition (bottom-left, the unique joint-uncertainty story), and the
  cumulative joint bounds (bottom-right). Computation is instant (arithmetic) but still runs through `WorkerRunner` for
  consistency (no progress bar). **Add** emits `{base}_JOINTUNC` (DERIVED, parent = randunc column); **Copy Python** ->
  `jointunc_to_code`. All computation is library work.
...
- **ML gap-filling tabs — `MlGapFillingTab` template** (`tabs/_ml_gapfilling_base.py`). The XGBoost tab's whole
  ...
  **Model/Results sub-tabs** (`SubTabs`), the title bar (Copy Python at the far-right edge) with **Run / Add** mounted as
  ...
  - **Run button gates on valid inputs** (all gap-filling tabs): disabled until the inputs are complete, via an
    `_inputs_valid()` / `_update_run_enabled()` pair driven by the existing `_update_status` hub (fires on every
    target/driver/feature change + on data load). The **ML tabs** need a target **+ ≥1 feature**; the **MDS tab** needs a
    target **+ 3 distinct valid drivers** (SWIN/TA/VPD). `_set_running(False)` re-gates on validity instead of blindly
    re-enabling.
...
**Last Updated:** 2026-06-21 | **Version:** v0.91.0 | **Package Manager:** `uv`
