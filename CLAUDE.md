...
|---|---|---|
| `NighttimePartitioningOneFlux` / `partition_nee_nighttime_oneflux` | `*_NT_OF` | ONEFlux Reichstein 2005, per-calendar-year, incl. robust `*_NT_OF_ROB`; helpers `lloyd_taylor`, `sunrise_sunset`. **Index and results stay TIMESTAMP_MIDDLE; internally the hour/day of year are the period END's**, as in ONEFlux and the DT_OF port (`end = index + dt/2`), with the year-end day-of-year wrap repaired. Reading the MIDDLE stamp literally moved E0 ~7 K off native ONEFlux. Missing `sw_in` stays night-eligible (ONEFlux's -9999 passes `< 10`). The sunrise/sunset comparison runs in float32: on day 80 sunrise is 5.99999990 in f8 but 6.0 in f4, which decides one record. Rref knot at `int(mean(idx))` (truncation). CH-DAV 2016 vs native ONEFlux 1.3.7-rc: RECO RMSE 0.0003, E0 within 0.05 K; the fits themselves stay float64 |
...
| `DaytimePartitioningOneFlux` / `partition_nee_daytime_oneflux` | `*_DT_OF` | ONEFlux `flux_part_gl2010` / Lasslop 2010 FLUXNET2015. **Day/night split is measured-`Rg`≤4/>4, NO solar geometry.** `nee`/`ta`/`sw_in` measured + `ta_f`/`sw_in_f`/`vpd` filled, `vpd_in_kpa=True`; emits `SE_GPP_DT_OF` + central `ALPHA`/`BETA`/`K`/`RREF`/`E0_DT_OF`. float32 working arrays **and parameter table** (FLOAT_PREC — the table's dtype decides window acceptance: `float32(0.01) != 0.01`, so ONEFlux's alpha-at-starting-guess guard never fires; `_check_parameters` widens to f8 so NEP 50 can't re-break it). **That guard is a real ONEFlux 1.3.7 bug** (its comment and the PV-Wave original reject such windows); on CH-DAV 2016 fixing it in ONEFlux itself raises annual DT GPP 5.7 % (7 windows). diive reproduces the bug by default so `*_DT_OF` matches ONEFlux output; opt-in `reject_alpha_at_start=True` applies the intended check (class + functional wrapper only; the L4.2 chain and GUI tab use the default). A missing *gap-filled* driver gives NaN RECO (needs `ta_f`) / GPP + SE (need `sw_in_f`, `vpd`), not a model value at the -9999 sentinel. Uncertainty look-up passes `edge='clip'` (see MDS below). Year-end `julday` wrap repaired as in `create_data_structures`. Model evaluations in the fits widen the float32 drivers to f8 first (`_build_predict`), as ONEFlux does (`.astype(DOUBLE_PREC)`); in f4 the residuals shift ~1e-5 relative, enough to flip the sign of a `k` that converges to 0, and the cascade branches on that sign. CH-DAV 2016 vs native ONEFlux 1.3.7-rc: same 143 windows, GPP r 0.9999, RECO r 0.9996, annual GPP −0.06 %. **The residual is ONEFlux's own non-reproducibility, not a port difference:** where `k` ≈ 0, its sign (so the model path) is decided by rounding, and ONEFlux run under two NumPy/SciPy versions differs in 6 windows and 0.7 % annual GPP on a gappy CH-DAV year — more than diive differs from either. Don't chase it further; ~5 s/year (ONEFlux ~24 s) |
...
- **Shared variable list: `widgets/variable_panel.py` (`VariablePanel`).** Every tab's left-hand list MUST be this component (filter + `VariableList` + `VariableDelegate` pills + fzf-style fuzzy filtering). Filter is subsequence-gated, *scored* (`_fuzzy_score`) and reorders best-match-first; clearing restores dataset order (`ORDER_ROLE`). Width is a shared setting (`theme.manager.list_width`) — don't set per-tab widths. `run_with_loading(name, fn)` paints a static busy cue before matplotlib's synchronous render freezes the loop; a call while one is queued replaces its `fn` (one render per burst), `flush_pending()` runs it at once. Also exposes `scroll_to(name)`, `clear_filter()`.
- **Registry-driven tabs.** `MainWindow` iterates `registry.TAB_CLASSES` (always-on: Overview, Log) and knows nothing about concrete tabs. Add a feature area = write a `DiiveTab` (`title` + `build()`) and register it. **Menu tabs are imported on first open:** `MENU_TABS`/`MENU_TAB_CLASSES` hold `LazyTab("diive.gui.tabs.<module>:<Class>", *args)` factories (calling one imports the module and returns a new tab; `.load()` gives the class), so `import diive.gui.app` pulls in no xgboost/sklearn/statsmodels/flux chain. Only `TAB_CLASSES` is imported eagerly. A menu tab must live in `diive.gui.tabs`, which `packaging/diive_gui.spec` bundles with `collect_submodules`.
...
- **Tab UX.** Movable, renamable (left double-click → `_rename_tab`), custom "×" (`icons.close_icon`); `tabBarDoubleClicked` fires for any button so an `eventFilter` records the double-click button (ignore middle/right). Studio tabs are Firefox-style pills. **Per-tab pin/freeze:** right-click a menu tab → Pin; pinned tabs (`_pinned`) skipped by `_push_data` (keep their dataset, cheap CoW), show a pin glyph. **Hidden tabs are not pushed to:** `_push_data` delivers only to the visible tab and marks the rest `_data_stale`; `_on_current_tab_changed` catches them up when shown. Code that reaches into a hidden tab must first make it current (`setCurrentWidget`, as `_open_menu_tab` does for single-instance tabs). Overview/Log never pinnable. **Freezing depends on every writer rebinding rather than mutating** — `_add_features` and `_sync_event_columns` used to assign columns in place, which a pinned tab's frame saw anyway; both now go through `assign`, matching the drop path. A new writer that does `df[col] = ...` silently un-freezes every pinned tab.

### Shared templates (reuse these — don't hand-roll layout/flow)

- **`widgets/worker.py` (`WorkerRunner`)** — `QObject` with `done(object)`/`failed(str)` + `is_running` guard; owns daemon-thread spawn + exception→`failed`. The outcome crosses back to the GUI thread on a private queued signal and **`is_running` clears there, immediately before `done`/`failed` fires** — it used to clear on the worker thread first, leaving a window where the guard read False while the result was still unhandled. So the flag stays True for that extra hop; every call site uses it only as "skip while a run is in flight", which is unaffected. A tab that hand-rolls its own `threading.Thread` gets **none** of this and needs its own guard (`_screening_base` does). Tab keeps `self._runner`, connects signals, calls `self._runner.run(self._compute_payload, *args)` where `_compute_payload` is a **pure** function (no Qt/threading). The outlier/correction/ustar tabs keep a thin synchronous `_worker(*args)` shim because their tests drive it directly. **`LatestRunner`** (same module) wraps it for views that recompute per selection: one job at a time, a request made while one runs replaces the queued one, only the newest request's result is delivered (`done`/`failed`), `cancel()` drops both, `settled` fires after them once idle. Its inner `WorkerRunner` is deliberately parentless so the worker can still emit after its owner is gone. **`ProcessLatestRunner`** is `LatestRunner` with each job sent through `run_in_process` to one shared `spawn` worker process (`process_pool()`, a `ProcessPoolExecutor(max_workers=1)` created on first use and reused; `shutdown_process_pool()` terminates it, hooked to `app.aboutToQuit` in `run()`, not to `closeEvent`). For computes that hold the GIL (statsmodels' STL), where a thread still freezes the window. The first job pays the worker start (~2 s).
- **`widgets/debounce.py` (`Debouncer`)** — for a control whose change signal fires per spin step or keystroke and drives a slow slot (recompute/re-render): connect the signal to `debouncer.trigger`, the slot runs once the value settles (`ms`, default 250). Holds the slot as a bound method (weak), so no lambda leak. Tests call `flush()`; a direct run of the slot should `cancel()` a pending one. Don't use it where a test or caller needs the result synchronously — Combine variables instead redraws at once for a valid colormap name and debounces only half-typed ones.
...
- **`tabs/_explorer_base.py` (`SingleVariableExplorerTab`)** — base for "pick one variable left, compute view right" tabs (Driver explorer, Gaps & coverage, Seasonal trend, Spectrogram, 3D surface). Owns the split skeleton, `select → run_with_loading → _compute`, default-variable picking (`default_var`/`default_numeric_only`, override `_default_variable`), `_recompute()`, opt-in stats strip, opt-in variable-list header (set `list_title`/`list_hint`; default None = no header), opt-in draggable list (`list_draggable`, for tabs that drag names onto drop targets, e.g. the X/Y/Z surface). Concrete tab overrides `_build_right()` + `_compute()`; selected var is always `self._target`. **Opt-in worker path:** a tab that overrides `_compute_payload` computes on a `LatestRunner`: `_compute_request()` reads widgets/data on the GUI thread and returns the arguments, the pure staticmethod `_compute_payload(*request)` runs on the worker (no Qt, no matplotlib), `_render_payload(payload)` draws on the GUI thread, optional `_render_error(message)`; `_computing()` tells view-only controls to skip redrawing while a newer result is on its way. A data push cancels the running job; a result arriving after the tab's widgets are gone is dropped. These tabs set a busy cursor on their own root widget, not the app-wide one. Driver explorer, Seasonal trend and Spectrogram use it; Gaps and the 3D surfaces stay synchronous. Tests wait with `_wait_for_worker(tab)`. **`use_process = True`** routes `_compute_payload` to `ProcessLatestRunner` (only Seasonal trend opts in): the payload must stay a staticmethod reachable by name, request and result must pickle, its console output does not reach the Log tab, and tests cannot monkeypatch it (patch `_compute_request` instead). `tests/test_gui.py` stops the worker in the module fixture `_worker_process`, which also asserts no child processes remain.
...
- **Overview** (`tabs/overview.py`) — first tab, focused on every load. 3×6 GridSpec (`_PANELS`) + `sstats` ribbon. **Linked zoom:** three datetime panels share x; `xlim_changed` clips the heatmap at once and recomputes the diel cycle and histogram once the view settles (`_zoom_debounce`; `_on_canvas_draw` restarts a pending one, since a pan repaint outlasts the settle time). The heatmap is an `AxesImage` (`HeatmapDateTime.plot(as_image=True)`); a long time series is drawn thinned with `decimate_line` (`_TS_COLUMNS_PER_PIXEL = 2`, full record kept in the line's `_diive_hover_xy`) and re-thinned on zoom and resize. One figure draw per select. **Lower-panel decorations stay inside their panel:** the time series spans the five lower columns, so constrained layout applies the widest lower-panel overhang to every lower column, and overhanging legends/tick labels collapsed the row at laptop widths. The diel and distribution legends are rebuilt compact and kept out of the layout (`_compact_legend` + `_LegendFitGuard`, which picks the full legend, coloured month names, or none at draw time); hour ticks (`_HourLocator`) and date ticks (`_FittedDateLocator`, min 3) follow the panel width; `_CompactFormatter` shortens values >= 10,000 to k/M. Repaint via `draw_idle()` (the deliberate exception to "use `draw()`"); `MplCanvas._solve_layout` runs two constrained-layout passes, at once on the first resize after a render and debounced after that. The render is debounced too: the canvas is a `_ResizeDeferringCanvas` (`FigureCanvasQTAgg` subclass) that drops the draw matplotlib queues per resize event of a visible canvas and paints a copy of the last frame scaled to the new size until the settle timer renders once (`auto_layout=False` canvases get the same deferred render). A hidden canvas still renders on its single show-time resize. The time-series panel draws per-record markers only up to `_MARKER_MAX_POINTS` (isolated values keep one). `on_data_loaded` diffs `created`, clears the fuzzy filter, `scroll_to`s + auto-selects new columns. Holds no subset state.
- **Hover tooltip** — `MplCanvas` attaches `HoverAnnotator` (`widgets/hover.py`); lines snap via `argmin` on `get_xdata(orig=False)` (unit-converted floats, NOT raw datetimes); `pcolormesh` (heatmap) reads `get_coordinates()`/`get_array()`, a 2-D `AxesImage` heatmap its extent and array; a line carrying `_diive_hover_xy` (thinned for drawing) snaps to that full record instead; scatter `PathCollection` snaps to the nearest point within `_SCATTER_PICK_RADIUS` and shows x/y(/z). Scatter pixel positions are cached per draw (sorted by x) for the pick. The hover skips blitting while its cached background's size differs from the figure's (a resize is still settling). Per-axis tag `_diive_hover_intlabel` formats an integer axis (year/month heatmap) as `Month 5`/`Year 2018` instead of misreading it as a clock time. Blits.
...
- **Driver explorer** (`tabs/drivers.py`, single-instance) — `dv.analysis.rank_drivers(...)` → `[DRIVER, CORR, ABS_CORR, BEST_LAG, N]` on the worker path; `ScatterXY` shifted by `BEST_LAG` (drawing that 175k-point scatter now costs ~0.36 s, mostly the markers; the legend search is ~0.02 s).
...
- **Events** — `gui/events.py` `EventManager` (`events.manager`); model is the library's `dv.events`. **Add event…** dialog result method is `make_event()`, NOT `event()` (which overrides `QDialog.event`). Events tab `tabs/events.py` (reflowing cards; Manage categories… — last category undeletable). `_sync_event_columns` (on `changed`) reconciles `EVENT_<name>` 0/1 columns. Overview overlays via `overlay_events(axis='x'/'y')`; `overlay_events` starts its zorders one above the highest image or mesh on the axes (heatmap cells are zorder 99), line panels keep 2/3/4. Category colour overrides via `Event.resolved_color(i, colors=)` / `overlay_events(..., colors=events.manager.categories)`.
- **Projects** — a `<name>.diive` folder (`__diive__` marker, `project.json`, `data.parquet`). Format is library's `diive.core.io.project` — serializes the **full** `MetadataStore` + an opaque `extras` dict (library never imports GUI types). GUI (`File ▸ Save project` Ctrl+S / Save as… / Open…) puts site, range, `created`, open tabs (label/title/pinned + each `save_state()`). Every `DiiveTab` has `save_state()`/`restore_state()` (default no-op) capturing *inputs only* (via `widgets/state_utils`); heavy results re-compute. Wrapped in try/except. `_open_project` loads clean then overlays `store.load_dict`; a plain load clears `_project_dir` so Ctrl+S can't overwrite a project with unrelated data. **File access runs on a worker:** open/save project, export and the startup load go through `MainWindow._run_io(busy, fn, args, on_done, on_failed)`; only the read/write runs on the thread, every state change happens in `on_done` on the GUI thread. While a job runs the load/save/export entries are disabled and the header shows what is happening; a save writes a snapshot taken when it was requested; "Save as" adopts the new project only after the write succeeded; `closeEvent` waits for a running save. Synchronous paths remain for tests/scripts (`_write_project`, `_load_project_folder`); tests call `_wait_for_io()` after `_open_project()`/`_save_project()`/`_save_file()`.
...
- **Splash / startup** — `gui/splash.py` `QPainter`-drawn spinner. `launch()` (`gui/__init__.py`) creates the app (`_create_application`: AppUserModelID, Fusion, icon) and paints the splash **before** importing `diive.gui.app`; `run(app=None, splash=None)` still works bare. `run()` builds `MainWindow(cfg, autoload=False)` then defers `window._initial_load(on_finished=…)`, which reads the data on the worker (else a blocking load freezes the spinner); the splash closes once the data is shown, and a failed startup load shows an error dialog. Tests build `MainWindow()` (`autoload=True`, synchronous). `_initial_load()` reopens `config.last_project` (if still a diive project) else the bundled example.
...
**Legend `loc='best'` on large collections.** `default_legend` returns an `_ArrayOffsetsLegend`: above `_LEGEND_BEST_MAX_POINTS` (10,000) points in one collection, its 'best' search keeps the points in one array (a copy of matplotlib's private `Legend._auto_legend_data`; falls back to matplotlib's method if a private name disappears). Placement is identical to matplotlib's and still re-placed on zoom; `test_array_legend_data_mirrors_matplotlib` guards the copy. Don't replace it with a subsample or a fixed `loc`.
...
- **`ScatterXY` artists.** Uncoloured points are a `_HollowMarkerCollection` (hollow markers take matplotlib's single-marker `draw_markers` path; markers snap to pixel centres). Colour-coded points are an `_OccludedMarkerCollection`: under Agg, markers fully covered by later opaque markers are left out of each draw, the image is identical pixel for pixel and the arrays are restored afterwards; vector output keeps every marker. Grouping colour-coded points by colormap entry was measured and rejected: points are drawn in data order, so grouping changes the stacking, and it is no faster. Find scatter collections with `isinstance(c, PathCollection)`, never by class name.
...
**Last Updated:** 2026-09-24 | **Version:** v0.91.1 | **Package Manager:** `uv`
