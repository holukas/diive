...
- **Driver explorer** (`tabs/drivers.py`, single-instance) — `dv.analysis.rank_drivers(...)` → `[DRIVER, CORR, ABS_CORR, BEST_LAG, N]` on the worker path; `ScatterXY` shifted by `BEST_LAG` (drawing that 175k-point scatter now costs ~0.7 s, most of it the legend's `loc='best'` search).
