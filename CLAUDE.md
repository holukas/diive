...
- **Hover tooltip** — `MplCanvas` attaches `HoverAnnotator` (`widgets/hover.py`); lines snap via `np.searchsorted` on `get_xdata(orig=False)` (unit-converted floats, NOT raw datetimes); `pcolormesh` reads `get_coordinates()`/`get_array()`. Blits.
- **Plot menu = one closable `PlottingTab(plot_type, title)` per method** (no single Plotting tab), `registry.MENU_TABS["Plot"]`. Menu glyphs via `gui/icons.py::menu_icon(label)`. Add a method = factory + branch in `plotting._draw_one` + controls in `plot_settings`. Most pick comparison panels (Ctrl+click); Hexbin/Scatter/Wind rose pick by role (`_XYZ_TYPES`). Ridgeline/histogram/wind rose are single-figure (own gridspec/polar axes, `canvas.auto_layout=False`). **Params apply on the Update button**, not live (variable selection stays live). GUI-only presentation passes: Axes group (`_axes`→`_apply_axes`), Reverse colormap (`_reverse_cmap`), export DPI (`_SaveDpiToolbar`, default 150).
...
**Last Updated:** 2026-06-25 | **Version:** v0.91.0 | **Package Manager:** `uv`
