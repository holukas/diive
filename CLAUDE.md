...
- **Driver explorer** (`tabs/drivers.py`, single-instance) — `dv.analysis.rank_drivers(...)` → `[DRIVER, CORR, ABS_CORR, BEST_LAG, N]` on the worker path; `ScatterXY` shifted by `BEST_LAG` (drawing that 175k-point scatter, ~3 s, is now the tab's main GUI-thread cost).
