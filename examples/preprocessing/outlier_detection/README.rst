...
.. list-table::
   ...
   * - Quick QA/QC
     - Hampel
     - Fast, robust to skew, works with non-normal distributions
   * - Known physical limits
     - AbsoluteLimits
     - Simple, deterministic, no parameters to tune
   * - Normal distribution
     - zScore
     - Works well when data is normally distributed
   * - Varying patterns
     - LocalSD
     - Adaptive to local variability
   * - Abrupt changes
     - Incremental
     - Detects step changes, not global outliers
   * - Unknown patterns
     - LOF
     - Density-based, finds anomalies without assumptions
   * - Multiple tests
     - StepwiseOutlierDetection
     - Combine methods, each filters progressively
