"""
==========================
Potential Radiation Calculations
==========================
...
# %%
# Potential Radiation - Stull Method
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
# Calculate potential shortwave radiation using the standard Stull (1988) method.
...
# %%
# Potential Radiation - Equation of Time Method
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
# Calculate using the equation of time (EoT) method for alternative approach.
...
sw_in_pot_eot = dv.variables.potrad_eot(
    timestamp_index=df.index,
    lat=47.286417,  # Davos latitude
    lon=7.733750,  # Davos longitude
    utc_offset=1,  # Central European Time
...
print("\nPotential Radiation - Equation of Time (TOA)")
print("=" * 50)
...
# Method 1: Stull (1988)
potrad_stull = dv.variables.potrad(
    timestamp_index=df.index,
    lat=47.286417,
    lon=7.733750,
    utc_offset=1
...
potrad_eot_toa = dv.variables.potrad_eot(
    timestamp_index=df.index,
    lat=47.286417,
    lon=7.733750,
    utc_offset=1,
...
potrad_eot_clearsky = dv.variables.potrad_eot(
    timestamp_index=df.index,
    lat=47.286417,
    lon=7.733750,
    utc_offset=1,
...
# %%
# Heatmap Visualization of Annual Patterns
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
# Visualize daily and hourly patterns of potential radiation throughout the year.
