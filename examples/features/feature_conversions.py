...
Demonstrates unit conversions and variable transformations commonly used in
eddy covariance processing, including sonic temperature to air temperature
conversion and latent heat to evapotranspiration transformation.

Best for: Converting between different measurement units and variable types.
"""
...
# Sonic temperature measurements require correction for the effects of water vapor
# on sonic wave speed to derive true air temperature.
...
print(f"\nAir temperature range: {ta.min():.2f} to {ta.max():.2f} K "
...
# temperature, essential for converting latent heat flux to evapotranspiration.
...
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
# Convert latent heat flux to evapotranspiration rate (ET) by accounting for
# temperature-dependent latent heat. This example loads real flux tower data
# and compares calculated ET with EddyPro-derived values.
