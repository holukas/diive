...
diive ships faithful Python ports of three reference NEE partitioning routines:
...
The two nighttime methods fit the temperature response of nighttime NEE; the
daytime method fits a light-response curve to daytime NEE. They estimate the
...
This example runs all three on the same input and compares them against each
...
``GPP_DT_CUT_REF`` for the daytime method).
...
# Reference columns: nighttime methods share the NT reference, the daytime method
# has its own DT reference.
...
fig.suptitle('diive NEE partitioning: ONEFLUX nighttime vs REddyProc nighttime vs '
             'REddyProc daytime - daily means and cumulative (CH-DAV 2017)')
plt.show()
