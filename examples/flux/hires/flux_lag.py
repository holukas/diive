"""
===================================
Time Lag Detection (MaxCovariance)
===================================

Detect time lag between two variables using covariance analysis.
...
Best for: Identifying measurement delays in high-resolution eddy covariance systems.
"""
...
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
# Demonstrates time lag detection by creating synthetic turbulent wind (w) and
# scalar (c) data with a known time lag, then recovering that lag using MaxCovariance.
...
# %%
# Display lag detection results
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
# Show the detected time lag compared to the known lag in the synthetic data.
...
# %%
# Visualize covariance function
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
# Plot the covariance as a function of time lag to visualize the peak.
...
try:
    ...
    print(f"Plot info: {type(e).__name__}")

print("\n[OK] Time lag detection complete.")
