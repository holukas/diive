...
| [**partitioning_nighttime_comparison.py**](flux/partitioning/partitioning_nighttime_comparison.py) | Run both diive nighttime ports — diive (ONEFLUX method) `_NT_OF` and diive (REddyProc method) `_NT_RP` — on identical inputs and compare them against each other and the common REddyProc reference (daily-mean + cumulative panels for RECO and GPP); highlights how window geometry, day/night split, and per-year vs whole-record E0 make the two implementations of the same paper diverge |
