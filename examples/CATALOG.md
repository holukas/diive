...
| [**partitioning_nighttime_comparison.py**](flux/partitioning/partitioning_nighttime_comparison.py) | Run both diive nighttime ports (ONEFlux `_NT_OF` and REddyProc `_NT_RP`) on identical inputs and compare them against each other and the common reference; highlights how window geometry, day/night split, and per-year vs whole-record E0 make the two implementations of the same paper diverge |
