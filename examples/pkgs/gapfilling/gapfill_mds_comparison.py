"""
========================================
FluxMDS Performance: Original vs Optimized
========================================

Compare FluxMDS original and optimized implementations.

Demonstrates the performance improvement of the optimized FluxMDS implementation
(vectorized NumPy operations) compared to the original implementation
(row-by-row apply operations). Both versions produce identical results
within floating-point tolerance.

Key optimizations:
- Replace .apply(axis=1) with vectorized NumPy operations
- Use searchsorted() for O(log n) time-window lookups
...
def plot_comparison_results(original_time, optimized_time, speedup,
    ...
    ax5.set_ylabel('Optimized FluxMDS', fontsize=12, fontweight='bold')
...
# %%
# Run original FluxMDS implementation
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
# %%
# Run optimized FluxMDS implementation
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
validation_pass = (
    (pred_diff.max() < 1.0 if both_valid.sum() > 0 else True)
    and (sd_diff.max() < 1.0 if both_valid_sd.sum() > 0 else True)
    and quality_match
)
