...
- **Examples consolidation (Phase 2)** — 82 executable examples across 44 files. Consolidated all RandomForestTS/XGBoostTS examples from source to `examples/gap_filling/`. RandomForestTS includes: example_randomforest_nee_gapfilling() (production settings + heatmap + cumulative flux), example_quickfill_rapid_prototyping() (fast exploratory), example_randomforest_hyperparameter_optimization() (GridSearchCV tuning). XGBoostTS includes: example_xgboost_nee_gapfilling() (harmonized with RF for direct comparison). Three-way gap-filling comparison: `examples/gap_filling/comparison.py` comparing MDS vs Random Forest vs XGBoost with heatmap visualizations and cumulative carbon flux curves.
...
- **OptimizeParamsRFTS enhancements** — Added `report_optimization()` method providing clear output with: tested parameter ranges, best parameters, performance metrics (R², MAE, RMSE), top N parameter combinations, parameter sensitivity, and production-ready code snippet with recommendations.
