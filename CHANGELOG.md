...
- **Examples consolidation (Phase 2+3)** — 81 executable examples across 43 files. Consolidated all RandomForestTS examples from source to `examples/gap_filling/randomforest_ts.py`: example_randomforest_nee_gapfilling() (production settings + cumulative flux), example_quickfill_rapid_prototyping() (fast exploratory), and example_randomforest_hyperparameter_optimization() (GridSearchCV tuning). Added gap-filling method comparison: `examples/gap_filling/comparison.py` comparing MDS vs Random Forest with cumulative carbon flux curves.
