...
- **PySide6 desktop GUI** — new `diive.gui` subpackage, launched via the `diive-gui` console script (optional `gui`
  ...
  - **Per-method plot tabs** opened from the Plot menu (Heatmap date/time, Heatmap year/month, Time series, Diel cycle,
    Cumulative year, Ridgeline, Hexbin), openable multiple times (numbered) and closable, each with a live settings
    panel. Ctrl+click compares variables across panels (heatmaps side by side, the per-variable line plots stacked; the
    ridgeline is single-variable).
