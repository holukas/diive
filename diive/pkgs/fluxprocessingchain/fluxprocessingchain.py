def example():
    ...
    # save_as_pickle(outpath=r'L:\Dropbox\luhk_work\_temp', filename="data", data=df)
    ...
    olr.zscore_dtnt(threshold=5, showplot=True, verbose=True)
    olr.stl_iqrz(zfactor=1.5, repeat=False, showplot=True)
    olr.lof_dtnt(n_neighbors=None, contamination='auto', showplot=True, verbose=True)
    _df = olr.fulldf
    save_as_pickle(outpath=r'L:\Sync\luhk_work\_temp', filename="data_L3.2", data=_df)
    ...
    from diive.pkgs.qaqc.qcf import FlagQCF
    from diive.core.io.files import load_pickle, save_as_pickle
    _df = load_pickle(filepath=r"L:\Sync\luhk_work\_temp\data_L3.2.pickle")
    ...
    [print(c) for c in _df.columns if 'NEE' in c]
