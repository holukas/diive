"""
GUI.TABS._SCREENING_BASE: SHARED STEPWISE-SCREENING TAB BASE
===========================================================

The full screening experience shared by the **Stepwise screening** tab
(`tabs/stepwise.py`) and the **Meteo screening (database)** tab
(`tabs/meteo_screening.py`): chain several outlier tests on one variable as a
list of editable **method cards** and inspect what each step removes, plus
corrections and an overall **QCF** (`dv.qaqc.FlagQCF`).

It drives the library's `StepwiseOutlierDetection`: each committed step runs on
the data the previous steps already cleaned, so spikes are peeled off
...
class ScreeningTabBase(DiiveTab):
    """Chain outlier tests on one variable as cards; inspect removals + QCF.

    Base for the Stepwise screening and Meteo (database) screening tabs. Operates
    on ``self._df`` / ``self._var``; subclasses set the data source, the emitted
    columns (:meth:`_emit_frame`), and optionally extra inspector pages."""

    title = "Stepwise screening"
    #: Label of the right-hand action button and what it emits (subclass override).
    add_button_label = "Add cleaned + flags + QCF to dataset"
    #: Operation name recorded in the emitted columns' provenance.
    provenance_op = "Stepwise screening"
    ...
    def _inspector_pages(self) -> list:
        """Segmented inspector pages as ``(label, builder, width)``. Subclasses
        append pages (e.g. the database variant adds a Resample page)."""
        return [
            ("Outliers", self._build_outliers_page, self._INSPECTOR_W),
            ("Corrections", self._build_corrections_page, self._INSPECTOR_W),
            ("Report", self._build_report_page, self._INSPECTOR_W_REPORT),
        ]

    # --- inspector (segmented: Outliers / Corrections / Report [/ Resample]) ---
    def _build_inspector(self) -> QWidget:
        ...
        pages = self._inspector_pages()
        self._page_widths = [w for _, _, w in pages]

        seg = QHBoxLayout()
        seg.setSpacing(4)
        self._seg_btns: list[QPushButton] = []
        for i, (label, _builder, _w) in enumerate(pages):
            b = QPushButton(label)
            b.setCheckable(True)
        ...
        for _label, builder, _w in pages:
    ...
    def _set_inspector_page(self, idx: int) -> None:
        ...
        widths = getattr(self, "_page_widths", None)
        if widths and 0 <= idx < len(widths):
            self._inspector.setFixedWidth(widths[idx])
    ...
    def _build_top_bar(self) -> QHBoxLayout:
        ...
        self.add_btn = QPushButton(self.add_button_label)
    ...
    def _build_result(self) -> None:
        """Set ``self._result_df`` to the columns the 'Add' button emits. The
        actual columns come from :meth:`_emit_frame` so subclasses (e.g. the
        database variant) can emit a different set."""
        self._result_df = self._emit_frame()

    def _emit_frame(self):
        """Columns for 'Add' from the current run + corrections: all FLAG_*_TEST
        columns, the overall QCF flag, the QCF-filtered series, and (if any
        corrections are active) the corrected series. Works with only corrections
        (no outlier chain), and with only an outlier chain (no corrections).
        Returns a DataFrame (with ``attrs[ATTRS_KEY]``) or None."""
        ...
        if payload is not None:
            ...
            attrs[qcf.filteredseriescol] = provenance_attr(
                origin=MODIFIED, parent=var, operation=self.provenance_op,
                params=params, tags=["outliers-removed", "qcf"])
            for c in det.flags.columns:
                attrs[str(c)] = provenance_attr(
                    origin=DERIVED, parent=var, operation=f"{self.provenance_op} flag",
                    params=params, tags=["flag"])
            attrs[qcf.flagqcfcol] = provenance_attr(
                origin=DERIVED, parent=var, operation=f"{self.provenance_op} QCF",
    ...
    def _add_to_dataset(self) -> None:
        ...
        self.status.setText(self._added_message(list(self._result_df.columns)))
        self.add_btn.setEnabled(False)

    def _added_message(self, cols: list) -> str:
        """Status line after a successful 'Add' (subclass override for wording)."""
        parts = []
        if self._payload is not None:
            parts.append("flags + QCF + filtered series")
        if any(str(c).endswith("_CORRECTED") for c in cols):
            parts.append("corrected series")
        return f"Added {len(cols)} column(s) ({', '.join(parts)}) to the variable list."
