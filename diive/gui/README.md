...
| `widgets/column_picker.py` | `ColumnPicker` — spec-driven group of variable combos with green ✓/red ✗ availability markers, auto-seeded by name (`dv.variables.auto_pick_column`); `seed(columns)`/`picks()`/`combos()`/`refresh_availability()`, `changed` signal. Shared by the partitioning and random-uncertainty input pickers (tabs with convention-driven dynamic seeding keep their own) |
